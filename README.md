...
1. 見積PDFのファイル名から `quote_no` を採取して履歴へ逆入力する機能。ファイル名に日付が無いため紐づけ規則の要件整理が必要。
   下準備として `ReportLinker::quoteFilesFor()` で機械ごとの見積PDF（番号・更新日時）を列挙できる。
2. 「メーカー作業報告書見積り」フォルダ（業者別）の報告書の自動取込み（業者名→機種の対応表が必要）。
